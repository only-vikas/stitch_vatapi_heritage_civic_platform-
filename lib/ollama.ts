...
export interface HeritageTriageOutput {
  category: string;
  severity: string;
  jurisdiction: string;
  description: string;
...
export const FALLBACK_TRIAGE_RESULT: HeritageTriageOutput = {
 ...
 */
async function toBase64(input: any): Promise<string | null> {
 ...
 */
async function getAvailableModel(preferredVisionModel = 'llava'): Promise<string> {
  try {
 ...
 */
export async function analyzeHeritageIssue(
  imageFile?: any,
  voiceText?: string
): Promise<HeritageTriageOutput> {
  const promptText = voiceText && voiceText.trim()
    ? `Analyze the following heritage issue report from Bagalkote district:\n\nReport Description & Voice Note: "${voiceText.trim()}"`
    : 'Analyze the attached image evidence of a heritage conservation issue in Bagalkote district.';
 ...
 */
function normalizeCategory(raw: string, contextText?: string): string {
 ...
 */
function normalizeJurisdiction(raw: string, contextText?: string): string {
 ...
 */
function inferOfflineFallback(text?: string): HeritageTriageOutput {
 ...
 */
export function generateNodeHash(): string {
 ...
 */
export function calculateEscalationDeadline(severity: string): Date {
  const now = new Date();
